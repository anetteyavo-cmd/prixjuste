...
import {
  Home, Fence, Paintbrush, Zap, ArrowRight, ArrowLeft,
  Calculator, MapPin, Download, Share2, CheckCircle,
...
function calculateQuote(type: string, params: Record<string, string>, cityName: string): QuoteResult {
  ...
  if (type === "maison_m2") {
    ...
    const postes = [
      { label: "Fondation", min: 25000, max: 35000 },
      { label: "Gros œuvre", min: 80000, max: 100000 },
      { label: "Toiture", min: 30000, max: 45000 },
      { label: "Électricité", min: 15000, max: 25000 },
      { label: "Plomberie", min: 12000, max: 20000 },
      { label: "Finitions", min: 40000, max: 60000 }
    ]
    postes.forEach(({ label, min, max }) => {
      items.push({
        label,
        quantity: surface,
        unit: "m²",
        unitMin: Math.round(min * mult),
        unitMax: Math.round(max * mult),
        totalMin: Math.round(min * mult * surface),
        totalMax: Math.round(max * mult * surface)
      })
    })
  }
  ...
  if (type === "cloture_ml") {
    ...
    items.push({
      ...
      unitMin: 8000,
      unitMax: 12000,
      totalMin: 8000 * length,
      totalMax: 12000 * length
    })
    items.push({
      label: "Mur",
      quantity: length,
      ...
      unitMin: 25000,
      unitMax: 35000,
      totalMin: 25000 * length,
      totalMax: 35000 * length
    })
    items.push({
      label: "Portail",
      quantity: 1,
  ...
  if (type === "peinture") {
    ...
    items.push({
      ...
      unitMin: 800,
      unitMax: 1200,
      totalMin: 800 * surface,
      totalMax: 1200 * surface
    })
    items.push({
      label: "Main d'œuvre",
      quantity: surface,
      ...
      unitMin: 600,
      unitMax: 1000,
      totalMin: 600 * surface,
      totalMax: 1000 * surface
    })
  }
  ...
  if (type === "electricite") {
    ...
    items.push({
      ...
      unitMin: 15000,
      unitMax: 25000,
      totalMin: 30000 * rooms,
      totalMax: 50000 * rooms
    })
    items.push({
      label: "Prises",
      quantity: rooms * 4,
      ...
      unitMin: 8000,
      unitMax: 15000,
      totalMin: 32000 * rooms,
      totalMax: 60000 * rooms
    })
    items.push({
      label: "Tableau",
      quantity: 1,
  ...
  return {
    title: `Devis ${quoteTemplates.find(t => t.type === type)?.name || type}`,
    type,
    city: cityName,
    items,
    totalMin,
  ...
  return (
    <div className="min-h-screen bg-[#F6F8FA]">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="text-center">
            <Badge className="mb-4">
              <Sparkles size={14} className="mr-1" /> Générateur intelligent
            </Badge>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Étapes précédentes non modifiées */}
        {/* ... */}
          ...
          <div className="animate-fade-in-up">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" leftIcon={<Share2 size={16} />}>Partager</Button>
                <Button variant="outline" size="sm" leftIcon={<Download size={16} />}>PDF</Button>
              </div>
            </div>
            ...
            <Card className="p-6 mb-6 bg-gradient-to-r from-[#0F2A44] to-[#1F3A5F] text-white">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-white/70">Estimation totale</p>
                  <p className="text-3xl font-bold mt-1">{formatPrice(result.totalMin)} - {formatPrice(result.totalMax)}</p>
                  <p className="text-white/70 mt-2">{result.title} • {result.city}</p>
                </div>
                <div className="w-20 h-20 bg-white/10 rounded-2xl flex items-center justify-center">
                  {quoteIcons[result.type] || <FileText className="w-10 h-10" />}
                </div>
              </div>
            </Card>
                ...
                {result.items.map((item, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-[#0F2A44]">{formatPrice(item.totalMin)}</p>
                      <p className="text-sm text-gray-500">à {formatPrice(item.totalMax)}</p>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-6 pt-6 border-t flex justify-between text-lg">
                <span className="font-semibold">Total</span>
                <span className="font-bold text-[#0F2A44]">{formatPrice(result.totalMin)} - {formatPrice(result.totalMax)}</span>
              </div>
            </Card>

            {/* ✅ BOUTON SAUVEGARDE ICI */}
            <div className="mt-6 flex justify-center">
              <SaveQuoteButton
                secteur={result.title}
                ville={result.city}
                devis_data={result}
              />
            </div>
            ...
            <div className="mt-6 flex gap-4">
              <Button variant="outline" onClick={reset} fullWidth>Nouveau devis</Button>
              <Link href="/analyze" className="flex-1"><Button fullWidth>Analyser un devis</Button></Link>
            </div>
          </div>
        )}
        ...
        {isCalculating && (
          <div className="text-center py-12">
            <div className="w-16 h-16 mx-auto mb-4 relative">
              <div className="absolute inset-0 border-4 border-[#0F2A44]/20 rounded-full" />
              <div className="absolute inset-0 border-4 border-[#0F2A44] border-t-transparent rounded-full animate-spin" />
            </div>
        ...
        {step < 4 && !isCalculating && (
          <div className="mt-8 flex justify-between">
            ...
            <Button onClick={handleNext} disabled={(step === 1 && !selectedType) || (step === 2 && !selectedCity)} rightIcon={<ArrowRight size={18} />}>
